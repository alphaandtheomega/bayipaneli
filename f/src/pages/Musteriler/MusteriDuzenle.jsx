...
export default function MusteriDuzenle() {
  ...
  useEffect(() => {
    if (
      ...
      console.log("Müşteri il/ilçe bilgisi:", customer.il, customer.ilce);
      console.log("Müşteri yetkili bilgisi:", customer.yetkili);
  ...
  const handleMusteriMutation = useMutation({
    mutationFn: (musteriData) => {
      if (id) {
        ...
        // Yeni kayıt işlemi
        return axios.post("http://localhost:3001/api/musteriler", musteriData);
      }
    },
    onSuccess: (data) => {
      setSuccess(true);
