...
export default function MusteriDashboard() {
   ...
   const {
    ...
    queryKey: ["musteriler", id], // ID'yi queryKey'e ekleyin
    queryFn: async () => {
      try {
        const response = await axios.get(
          `http://localhost:3001/api/musteriler/${id}`
        );
    ...
    // Ağ bağlantısı geri geldiğinde otomatik yeniden çekmeyi devre dışı bırak    refetchOnReconnect: false,
  });  // Veri geldiğinde state'i güncelle
  useEffect(() => {
    if (musteriIdData && Array.isArray(musteriIdData) && musteriIdData.length > 0) {
      // API'den dönen veri bir dizi içinde nesne olduğu için [0] ile ilk öğeye erişiyoruz
      setMusteriUnvan(musteriIdData[0].unvan || "");
      console.log("Müşteri unvanı:", musteriIdData[0].unvan);
    } else if (musteriIdData && !Array.isArray(musteriIdData)) {
      // Doğrudan nesne ise
      setMusteriUnvan(musteriIdData.unvan || "");
      console.log("Müşteri unvanı:", musteriIdData.unvan);
    } else {
      console.log("Güncellenecek veri bulunamadı:", musteriIdData);
    }
  }, [musteriIdData]);

  const handleDelete = async () => {
    try {
      await axios.delete(`http://localhost:3001/api/musteriler/${id}`);
      ...
      <div className="bg-gradient-to-b from-white to-gray-50 rounded-lg shadow-md p-8 mx-auto max-w-7xl mt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-8">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 transform transition-all duration-200 hover:shadow-md hover:-translate-y-1">
            ...
            <p className="text-gray-800 font-semibold text-lg">{musteriIdData[0].yetkili || "-"}</p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 transform transition-all duration-200 hover:shadow-md hover:-translate-y-1">
            ...
            <p className="text-gray-800 font-semibold text-lg">{musteriIdData[0].eposta || "-"}</p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 transform transition-all duration-200 hover:shadow-md hover:-translate-y-1">
            ...
            <p className="text-gray-800 font-semibold text-lg">
              {musteriIdData && Array.isArray(musteriIdData) && musteriIdData[0] 
                ? `${musteriIdData[0].il} / ${musteriIdData[0].ilce}` 
                : "-"}
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 transform transition-all duration-200 hover:shadow-md hover:-translate-y-1">
            ...
            <p className="text-gray-800 font-semibold text-lg">
              {musteriIdData && Array.isArray(musteriIdData) && musteriIdData[0] ? musteriIdData[0].tel || "-" : "-"}
            </p>
          </div>
        </div>
