...
export default function Root() {
    ...
    const handleSignOut = async () => {
        try {
            ...
            const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/logout`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    useEffect(() => {
        ...
        const checkDbConnection = async () => {
            ...
            try {
                const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/db-status`);
                if (!response.ok) {
