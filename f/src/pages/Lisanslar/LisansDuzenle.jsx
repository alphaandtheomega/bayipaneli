...
export default function LisansDuzenle() {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        const response = await axios.get(
          `http://localhost:3001/api/paketler/${paketIdForQuery}`
        );
  ...
  const {
    ...
    queryFn: async () => {
      try {
        const response = await axios.get(
          `http://localhost:3001/api/lisans/${id}`
        );
        // API'den dönen veri dizi değilse diziye çevir
  ...
  useEffect(() => {
    if (
      lisansDuzenleData &&
      Array.isArray(lisansDuzenleData) &&
      lisansDuzenleData.length > 0 &&
      typeof lisansDuzenleData[0] === "object"
    ) {
      const lisanslar = lisansDuzenleData[0];
      // Paket id'sini state'e ata
      if (lisanslar.paket_id) setPaketIdForQuery(lisanslar.paket_id);
      let items = lisanslar.items || [];
      // Eğer items string ise diziye çevir
      ...
      // Modül adları ile eşleşmeyenleri filtrele (güvenli olsun diye)
      if (Array.isArray(modulPaketDuzenleData) && modulPaketDuzenleData.length > 0) {
        items = items.filter((item) => modulPaketDuzenleData.some((modul) => modul.modul_adi === item));
      }
      const formValues = {
        bayi_adi: lisanslar.bayi_adi || "",
        musteri_adi: lisanslar.musteri_adi || "",
        paket_adi: lisanslar.paket_adi || "",
        kullanici_sayisi: lisanslar.kullanici_sayisi || 1,
        lisans_suresi: lisanslar.lisans_suresi || 0,
        is_demo: lisanslar.is_demo || false,
        items: items,
        lisans_kodu: lisanslar.lisans_kodu || "",
        yetkili: lisanslar.yetkili || "",
        aktif: lisanslar.aktif || "",
        kilit: lisanslar.kilit || "",
      };

      console.log("Form değerleri yükleniyor:", formValues);
      form.reset(formValues);
    } else if (lisansDuzenleData) {
      // Hatalı veri geldi, logla
      console.error("Beklenen formatta lisans verisi gelmedi:", lisansDuzenleData);
    }
  }, [lisansDuzenleData, form, modulPaketDuzenleData]);

  // Lisans düzenle işlemi için useMutation kullanımı
  const handleLisansDuzenleMutation = useMutation({
    mutationFn: (LisansData) => {
      return axios.put(`http://localhost:3001/api/lisansduzenle/${id}`, LisansData);
    },
    onSuccess: () => {
      setSuccess(true);
      setError(null);
  ...
  function onSubmit(values) {
    const lisansKodu = lisansDuzenleData[0].lisans_kodu;
    const aktif = true;
    const kilit = false;
